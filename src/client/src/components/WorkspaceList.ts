...
export class WorkspaceList extends LitElement implements KeyboardNavigableSection {
  ...
  private renderHeading() {
    if (!this.collapsible) return html`<span>${this.sectionTitle ?? t("Workspaces")}</span>`;
    ...
    return html`<button class="section-toggle" aria-expanded=${String(!this.collapsed)} @click=${() => { this.onToggleCollapsed?.(); }}><span class="section-title"><span class="section-name">${this.collapsed ? "▸" : "▾"} ${this.sectionTitle ?? t("Workspaces")}</span>${this.collapsed ? html`<small class="section-selected" title=${selectedTitle}>${selectedSummary}</small>` : null}</span><small class="section-count">${this.workspaces.length}</small></button>`;
  }
